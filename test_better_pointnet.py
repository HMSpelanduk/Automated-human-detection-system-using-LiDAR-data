...
# ------------------------------
DBSCAN_EPS = 0.0325 #for drone (0.06~0.2)/for realsense (0.023~0.035) // if mannequin and background mix, decrease value
...
# Mini mannequin (~0.5m): eps2 target should be around 0.6cm–1.6cm.
# ------------------------------
ENABLE_STAGE2 = True
...
def stage2_refine_candidate(candidate_pts: np.ndarray):
    ...
    H = float(dims.max())  # size proxy (works well for your mini mannequin)
    ...
    for s in STAGE2_SHRINK_STEPS:
        ...
        if ENABLE_BRIDGE_REMOVAL:
            rad = max(VOXEL_SIZE, eps2)  # make bridge radius meaningful with your voxel grid
...
# ==============================
# DETECTION: Stage-1 -> Stage-2 -> choose best subcluster -> bbox
# ==============================
best = {
    "stage1_idx": None,
    "stage2_idx": None,
...
for i, cluster_pts in enumerate(clusters_list):
    ...
    # Pick best subcluster by mannequin probability
...
# bbox only for chosen mannequin points
if best["pts"] is not None and best["stage1_idx"] is not None:
    ...
    # highlight stage1 candidate in blue
